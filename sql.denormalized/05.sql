...
FROM (
  ...
  CROSS JOIN LATERAL jsonb_array_elements(arr.hashtags) AS elem(jshtag)
  WHERE
    to_tsvector(
      'english',
      COALESCE(t.data->'extended_tweet'->>'full_text', t.data->>'text')
    ) @@ to_tsquery('english', 'coronavirus')
    AND t.data->>'lang' = 'en'
) AS uniq
GROUP BY uniq.tag_text
ORDER BY count DESC, uniq.tag_text
LIMIT 1000;
